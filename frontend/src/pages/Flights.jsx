...
export default function Flights() {
  ...
  const [searched,    setSearched]    = useState(false); // đã tìm lần nào chưa
  ...
  useEffect(() => {
    if (!urlFrom && !urlTo) return; // chưa chọn gì thì không fetch
  ...
  const fetchFlights = async (params = {}) => {
    setLoading(true);
    setSearched(true);
    // Reset bộ lọc mỗi lần tìm mới
    setAirline('Tất cả');
    setTimeSlot(0);
    setMaxPrice(5000000);
    setSortBy('time');
    try {
      // Chỉ gửi param có giá trị
      const query = {};
  ...
  // Khi bấm Tìm trong form → cập nhật URL (useEffect sẽ tự fetch)
  ...
  // Lọc + sắp xếp phía client (realtime)
  const filtered = useMemo(() => {
    let list = [...flights];
    const slot = TIME_SLOTS[timeSlot];

    if (airline !== 'Tất cả') {
      list = list.filter(f => f.airline === airline);
    }
    if (timeSlot !== 0) {
      list = list.filter(f => {
  ...
  // Tên hiển thị điểm đi/đến trên tiêu đề
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="mb-6">
        ...
        {urlDate && (
          <p className="text-gray-400 text-sm mt-1">
            📅 Ngày: {new Date(urlDate).toLocaleDateString('vi-VN', { weekday:'long', day:'2-digit', month:'2-digit', year:'numeric' })}
          </p>
        )}
      </div>

      {/* Form tìm kiếm — đồng bộ giá trị từ URL */}
      <div className="mb-8">
        <SearchForm
      ...
      {/* Chưa tìm lần nào → hiển thị gợi ý */}
      {!searched && (
        <div className="text-center py-20 bg-white rounded-2xl border border-gray-100">
          <p className="text-5xl mb-4">✈️</p>
          <p className="text-gray-500 font-medium text-lg">Chọn điểm đi và điểm đến để tìm chuyến bay</p>
          <p className="text-gray-400 text-sm mt-2">Hơn 180 chuyến bay tới 36 thành phố trên toàn quốc</p>
        </div>
      )}

      {/* Đã tìm → hiển thị kết quả + bộ lọc */}
      {searched && (
                  ...
                  <div className="space-y-2">
                    {AIRLINES.map(a => (
                      <label key={a} className="flex items-center gap-2 cursor-pointer group">
                        ...
                        <span className={`text-sm ${airline === a ? 'text-primary font-medium' : 'text-gray-600'} group-hover:text-primary`}>
                          {a}
                        </span>
                      </label>
                    ))}
                  ...
                  <div className="space-y-2">
                    {TIME_SLOTS.map((slot, i) => (
                      <label key={i} className="flex items-center gap-2 cursor-pointer group">
                        ...
                        <span className={`text-sm ${timeSlot === i ? 'text-primary font-medium' : 'text-gray-600'} group-hover:text-primary`}>
                          {slot.label}
                        </span>
                      </label>
                    ))}
                ...
                <div>
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="text-xs text-primary font-bold">
                      {(maxPrice / 1000000).toFixed(1)}M đ
                    </span>
                  </div>
                ...
                {loading ? 'Đang tìm...' : (
                  <>
                    ...
                    {flights.length !== filtered.length && (
                      <span className="text-gray-400"> (trong {flights.length})</span>
                    )}
                  </>
                )}
              </p>
            ...
            {!loading && filtered.length === 0 && (
              <div className="text-center py-20 bg-white rounded-2xl border border-gray-100">
                ...
                <p className="text-gray-600 font-semibold text-lg">
                  Không tìm thấy chuyến bay {urlFrom} → {urlTo}
                </p>
                <p className="text-gray-400 text-sm mt-2 mb-5">
                  {activeFilters > 0
                    ? 'Thử xóa bộ lọc để xem thêm kết quả'
                    : 'Tuyến bay này chưa có chuyến hoặc không bay vào ngày đã chọn'}
                </p>
                {activeFilters > 0 && (
